...
#definiton for finding bounded box which has the maximum area
def max_area(arr):
    area = 0
    coord = 0
    areas = []
...
#cap = cv2.VideoCapture(0) #taking computer's camera
cap = cv2.VideoCapture("http://a:1@192.168.43.1:8080/video") # taking phone's camera
...
while True:
    _, im = cap.read()
    bbox, label, conf = cv.detect_common_objects(im) #object detection functions
    print(bbox)
    ...
    #write output texts with label, coordinates and max area values. Then use it in mobile
